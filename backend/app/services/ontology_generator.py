...
Analyze text content and generate entity and relationship type definitions suitable for social simulation.
Adapted from MiroFish OASIS logic for NovelSwarm.
"""
...
logger = logging.getLogger("novelswarm.ontology")
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        # If text exceeds 50,000 characters, truncate (only affects LLM input)
    ...
    def _validate_and_process(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for entity in result.get("entity_types", []):
            ...
            if len(str(entity.get("description", ""))) > 100:
                entity["description"] = str(entity["description"])[:97] + "..."
        ...
        for edge in result.get("edge_types", []):
            ...
            if "attributes" not in edge:
                edge["attributes"] = []
            if len(str(edge.get("description", ""))) > 100:
                edge["description"] = str(edge["description"])[:97] + "..."
        ...
        entity_names = {e.get("name") for e in result.get("entity_types", []) if isinstance(e, dict)}
